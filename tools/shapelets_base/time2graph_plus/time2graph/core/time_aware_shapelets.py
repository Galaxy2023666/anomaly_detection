    ...
    while cnt < max_iters:
        ...
        for x, y in dataloader:
            ...
            for k in range(1, len(lb_idx)):
                ...
                # todo
                # 如果是1维且长度为1时，标准差为空
                # if (len(src.shape) == 1 and src.shape[0] == 1) or (len(dst.shape) == 1 and dst.shape[0] == 1):
                if torch.std(src) == 0.0 or torch.std(dst) == 0.0:
                    print(src)
                    print(dst)
                    print('\n')
                elif (len(src.shape) == 1 and src.shape[0] == 1) or (len(dst.shape) == 1 and dst.shape[0] == 1):
                    print(src)
                    print(dst)
                    print('\n')
                    # loss -= torch.abs(torch.distributions.kl.kl_divergence(
                    #     Normal(torch.mean(src), src),
