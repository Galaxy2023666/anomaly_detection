    ...
    def __main__(pid, args, queue):
        ...
        for time_series_segments in args:
            ...
            for cnt in range(n_candiates):
                ...
                # todo
                if len(time_series_segments[update_idx]) == 0:
                    continue
...
def generate_shapelet_candidate(time_series_set, num_segment, seg_length, candidate_size, **kwargs):
    ...
    for idx in range(num_segment):
        args_tmp.append(time_series_set[:, idx, :, :].reshape(num_time_series, -1))

    # todo 有些异常点出现在分割点，需要补充跨分割点的shapelet

    # todo 过滤不太合理的shapelet
    for idx_shapelet in range(len(args_tmp)):
        args_shapelet = args_tmp[idx_shapelet]
        index_delete = []
        for idx_ts in range(len(args_shapelet)):
            count_dict = {}
            arg_ts = args_shapelet[idx_ts]

            for arg_value in arg_ts:
                if arg_value in count_dict:
                    count_dict[arg_value] += 1
                else:
                    count_dict[arg_value] = 1
            count_list = sorted(count_dict.items(), key=lambda s: -s[1])

            if count_list[0][1] / len(arg_ts) >= 1.0:
                index_delete.append(idx_ts)

        # for index in index_delete:
        #     args.append(np.delete(args_shapelet, index, axis=0))
        args.append(np.delete(args_shapelet, index_delete, axis=0))
        print(np.delete(args_shapelet, index_delete, axis=0).shape)
    ...
    # ret = work_func(args, n_clusters, seg_length)
...
def graph_embedding(tmat, num_shapelet, embed_size, cache_dir, **deepwalk_paras):
    ...
    for idx in range(tmat.shape[0]):
        ...
        cmd_python = '/Users/xiefang09/xiefang/python/time_series-20230328/time_series/tools/shapelets_base/weighted_deepwalk/__main__.py'
        deepwalk_cmd = [
            '{} --input {} --format weighted_edgelist --output {} --representation-size {}'.format(
                cmd_python, edgelist_path, embedding_path, embed_size)
        ]
