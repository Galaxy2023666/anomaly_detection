...
class TransformerVar(nn.Module):
    ...
    def __init__(self, win_size, enc_in, c_out, n_memory, shrink_thres=0,\
        ...
        # Encoder
        self.encoder = Encoder([EncoderLayer(AttentionLayer(win_size, d_model, n_heads, dropout=dropout), d_model, d_ff,
                                             dropout=dropout, activation=activation) for _ in range(e_layers)],
    ...
    def forward(self, x):
        """
        :param x: (input time window) : N x L x enc_in
        :return:
        """
        ...
        if self.memory_initial is not None:
            out = self.weak_decoder(out)
