...
class LinkageAnalysisBase(object):
    def __init__(self):
        return object.__init__(self)

    def calculate_relation(self, x, y):
        pass

# 动态时间规整
class FastDtwLinkageAnalysis(LinkageAnalysisBase):
    def __init__(self):
        return LinkageAnalysisBase.__init__(self)
...
# 时间滞后相关
class DelayTimeLinkageAnalysis(LinkageAnalysisBase):
    def __init__(self):
        return LinkageAnalysisBase.__init__(self)

    def calculate_relation(self, x, ym, lag):
        distance, path = fastdtw(x, y, dist=euclidean)

        return distance, path

class CoFluxLinkageAnalysis(LinkageAnalysisBase):
    coTHR = 0.8
    ...
    def calculate_relation(self, afx_set, afy_set, max_dis=None):
        result_set = []
        cnt = 0
        for afx in tqdm(afx_set):
            for afy in afy_set:
                if len(afx) == 0 or len(afy) == 0:
                    ...
                    result_set.append(
                        self.FCC(self.amplification(0.5, 10, afx), self.amplification(0.5, 10, afy), max_dis))
                cnt += 1
        maxv = 0
        minv = 0
        if len(result_set) == 0:
...
class PhaseSynchronizationLinkageAnalysis(LinkageAnalysisBase):
    def __init__(self):
        return LinkageAnalysisBase.__init__(self)
...
if __name__ == '__main__':
    ...
    for item in afy_set:
        cm = CoFluxLinkageAnalysis()
        (pos_neg, shift), ccv = cm.calculate_relation(afx_set=afx_set, afy_set=item)
        if pos_neg == 1:
            print("[pos] ", end="")
        elif pos_neg == -1:
            print("[neg] ", end="")
