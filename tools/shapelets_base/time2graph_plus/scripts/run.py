...
from tools.shapelets_base.time2graph_plus.config import model_args, module_path, xgb_args, njobs, STB
...
if __name__ == '__main__':
    ...
    if os.path.isfile('{}/scripts/cache/{}_x_train.npy'.format(module_path, args.dataset)):
    ...
    if os.path.isfile(shapelet_cache) and args.shapelet_cache:
    ...
    if os.path.isfile(model_cache):
        model.load_model(model_cache, map_location=torch.device('cpu'))
    else:
    ...
    shapelet_distance_cache = '{}/scripts/cache/{}_shapelet_distance_{}.cache'.format(module_path, args.dataset, tflag_str)
    if os.path.isfile(shapelet_distance_cache):
        shapelet_distance = torch.load(shapelet_distance_cache, map_location=torch.device('cpu'))
    else:
        shapelet_distance, _ = model.get_distance(x_train)
        torch.save(shapelet_distance, shapelet_distance_cache)

    from tools.shapelets_base.time2graph_plus.time2graph.core.model_embeds import Time2GraphEmbed

    t2g = Time2GraphEmbed(kernel=general_options['kernel'], seg_length=model_options['seg_length'])
    model_cache = '{}/scripts/cache/{}_t2g_{}.cache'.format(module_path, args.dataset, tflag_str)
    if os.path.isfile(model_cache):
        t2g.load_model(model_cache, map_location=torch.device('cpu'))
    else:
        t2g.load_shapelets(fpath=shapelet_cache)
        t2g.fit(x=x_train, y=y_train, n_splits=args.n_splits, reset=False, cache_dir='{}/scripts/cache'.format(module_path))
        t2g.save_model(model_cache)
    accu, prec, recall, f1 = evaluate_performance(y_true=y_test, y_pred=t2g.predict(x=x_test))
    Debugger.info_print('res: accu {:.4f}, prec {:.4f}, recall {:.4f}, f1 {:.4f}'.format(accu, prec, recall, f1))
