...
from tools.shapelets_base.shapelet_transformation.src import util
from tqdm import trange
import matplotlib.pyplot as plt
from tools.shapelets_base.shapelet_transformation.src.util import euclidean_distance, sdist_mv, euclidean_distance_shifted, max_corr, mean_shift
import itertools
...
class RLS_candidateset():
    ...
    def get_neighborhood(self, j, L, epsilon, step=1, beta=0):
        ...
        '''
        "You can get a neighborhood only for a candidate set with empty scores"
        assert (self.scores is None or len(self.scores) == 0)
