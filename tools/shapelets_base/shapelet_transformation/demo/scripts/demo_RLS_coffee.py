...
from tools.shapelets_base.shapelet_transformation.src.preprocessing_anomaly import preprocessing_anomaly
from tools.shapelets_base.shapelet_transformation.src.SVDD.SVDD import SVDD
from tools.shapelets_base.shapelet_transformation.src.util import Scaler, euclidean_distance_shifted, length_normalized_distance, max_corr
from tools.shapelets_base.shapelet_transformation.src.searching.bruteforce_multivariate import Bruteforce_extractor_mv, Candidateset
from tools.shapelets_base.shapelet_transformation.src.searching.RLS import RLS_extractor, RLS_candidateset
...
data_name = "Coffee"
X_train = np.load(f'../data/{data_name}_train.npy')
y_train = np.load(f'../data/{data_name}_train_labels.npy')
X_test = np.load(f'../data/{data_name}_test.npy')
y_test = np.load(f'../data/{data_name}_test_labeles.npy')
...
# todo
# clsuter_id = pd.read_csv('/Users/xiefang09/xiefang/python/time_series-20230328/time_series/data/anomaly/cluster.csv')
# data = pd.read_csv('/Users/xiefang09/xiefang/python/time_series-20230328/time_series/data/anomaly/baseline_202306.csv')
#
# cols = ['指标']
# data = pd.merge(clsuter_id, data).query("cluster_id == 1 or cluster_id == 4")
# value_cols = data.columns[5:]
# for value_col in value_cols:
#     data[value_col] = data[value_col].apply(convert_to_zeros)
# row_number = data.shape[0]
# for i in range(row_number):
#     if data.iloc[i, 5:].max() - data.iloc[i, 5:].min() == 0.0:
#         continue
#     else:
#         data.iloc[i, 5:] = (data.iloc[i, 5:] - data.iloc[i, 5:].mean()) / (data.iloc[i, 5:].std())
#
# data['cluster_id'] = data['cluster_id'].apply(lambda x: 0 if x == 1 else 1)
#
# index_drop = drop_useless_samples(data.iloc[:, 5:], [], 2, 0.4)
# print(index_drop)
# data_train = data.drop(index_drop)
#
# data_train = data.iloc[0: int(data.shape[0]*0.8), ]
# data_test = data.iloc[int(data.shape[0]*0.8):, ]
#
# X_train_anomaly, y_train_anomaly = data_train.values[:, 5:].astype(np.float).reshape(-1, data.shape[1] - 5, 1), \
#                    data_train.values[:, 4].astype(np.int)
# X_test_anomaly, y_test_anomaly = data_test.values[:, 5:].astype(np.float).reshape(-1, data.shape[1] - 5, 1), \
#                  data_test.values[:, 4].astype(np.int)
...
for seed in range(2):
    ...
    plt.legend()
    plt.title('{}: The extracted shapelets'.format(seed), fontweight="bold")
    plt.show()
...
#
# plt.contourf(xx, yy, Z, levels=[0, Z.max()], colors="palevioletred")
# plt.scatter(X_test_transform[pIndex_test, 0], X_test_transform[pIndex_test, 1], facecolor='C0', marker='o', s=100,
#             linewidths=2, edgecolor='black', zorder=2, label="Normal")
#
# plt.scatter(X_test_transform[nIndex_test, 0], X_test_transform[nIndex_test, 1], facecolor='C3', marker='o', s=100,
#             linewidths=2, edgecolor='black', zorder=2, label="Anomalies")
#
# plt.legend(ncol=1, loc='upper left', edgecolor='black', markerscale=1.2, fancybox=True)
#
# plt.xlim((-0.01, y_max))
# plt.ylim((-0.01, y_max))
# plt.ylabel("shapelet 2")
