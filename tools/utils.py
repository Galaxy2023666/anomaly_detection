...
def k_means_clustering(x, n_mem, d_model, device=torch.device('cuda:0')):
    start = time.time()
...
class OneEarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, path):
        ...
        torch.save(model.state_dict(), '{}_checkpoint_{}.pth'.format(path, self.type))
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, val_loss2, model, path):
        if self.verbose:
            print(f'Validation loss decreased ({self.val_loss_min:.6f} --> {val_loss:.6f}).  Saving model ...')
        torch.save(model.state_dict(), '{}_{}'.format(path, 'checkpoint.pth'))
        self.val_loss_min = val_loss
        self.val_loss2_min = val_loss2