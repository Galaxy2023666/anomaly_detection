...
from tools.shapelets_base.time2graph_plus.baselines.feature_based import FeatureModel
from tools.shapelets_base.time2graph_plus.time2graph.utils.base_utils import Debugger
...
class Time2GraphGAT(ModelUtils):
    ...
    def get_distance(self, X, train=False):
        __shapelet_distance = shapelet_distance(
            time_series_set=X, shapelets=self.shapelets, seg_length=self.seg_length,
            tflag=self.tflag, tanh=self.kwargs.get('tanh', False), debug=self.debug,
            init=self.init, warp=self.warp, measurement=self.measurement)
    ...
    def fit(self, X, Y, n_splits=5, reset=False):
        ...
        self.shapelet_distance, X_adj = self.__gat_features__(X_scale)
        ...
        if self.out_clf:
            if not self.ft_xgb:
                ...
                self.xgb.fit(self.__gat_hidden_feature(feat=self.shapelet_distance, adj=X_adj, X=X, train=True), Y)
            else:
        ...
        y_pred = self.___gat_predict(feat=self.shapelet_distance, adj=X_adj)
        accu, prec, recall, f1 = evaluate_performance(y_pred=y_pred, y_true=Y)
        Debugger.info_print('fully-connected-layer res on training set: {:.4f}, {:.4f}, {:.4f}, {:.4f}'.format(
            accu, prec, recall, f1))
        if self.out_clf:
            y_pred = self.xgb.predict(self.__gat_hidden_feature(feat=self.shapelet_distance, adj=X_adj, X=X))
            accu, prec, recall, f1 = evaluate_performance(y_pred=y_pred, y_true=Y)
