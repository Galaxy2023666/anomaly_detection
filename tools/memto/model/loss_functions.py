...
class GatheringLoss(nn.Module):
    ...
    def forward(self, queries, items):
        '''
        Latent Space Deviation (LSD) LSD(qts, m) at time point t as distance between each query qts and its nearest
        memory item ms, pos t in latent space
        queries : N x L x C
        items : M x C
        '''
        batch_size = queries.size(0)
        ...
        # gathering_loss = gathering_loss.contiguous().view(int(gathering_loss.shape[0]/batch_size), -1)
