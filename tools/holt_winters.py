...
class HoltWinters(object):
    ...
    def __init__(self, p, test_num, sp=2, mtype='additive'):
        self.ts = None
        self.p = p
        if test_num <= 0:
    ...
    def Mape(self):
        ...
        if self.pred is None:
            raise Exception("You must run function predict_values before" +
    ...
    def grid_search(self, ts, alphas=None, betas=None, gammas=None, err_type="MSE"):
        """
        @params:
        ...
        self.ts_train = ts[:-self.test_num]
        self.ts_test = ts[-self.test_num:]
        ...
        for alpha, beta, gamma in grids:
            ...
            y_pred = self.predict_values(self.test_num)
        ...
        print("best error: {}".format(self.best_err))
...
if __name__ == '__main__':
    data = pd.read_csv('/Users/xiefang09/xiefang/python/time_series/data/prediction/quit_rate.csv')
    data.loc[:, 'Month'] = pd.to_datetime(data.loc[:, '月份'], format='%Y%m')

    HWmodel = HoltWinters(12, 5)

    # plt.plot(np.array(HWmodel.ts_test))
    # plt.plot(HWmodel.best_pred)
    data_ts = data.sort_values(by='Month').groupby(['序列']).apply(
        lambda x: HWmodel.grid_search(pd.Series(x['自愿离职率'].values, index=x['Month'].values))
    ).reset_index()
