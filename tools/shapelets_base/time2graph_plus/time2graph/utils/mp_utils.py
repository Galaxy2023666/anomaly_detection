...
class ParMap(object):
    ...
    def run_slices(self, slices):
        ...
        if self.monitor_func is not None:
            monitor = mp.mp.Process(target=self.monitor_func, args=(report_queue,))
            monitor.start()
        else:
            monitor = None
        ...
        if self.njobs == 1:
            ...
            with contextlib.closing(mp.mp.Pool(self.njobs, maxtasksperchild=self.__mtpc)) as pool:
...
def func_wrapper(args):
    ...
    return func(mp.mp.current_process().ident, *args[1:])
